...
subtitle: WEEK `r if( lubridate::isoweek(lubridate::today()) == 1) { 52} else { lubridate::isoweek(lubridate::today())-1 }  `/2022
date: Regional Bureau for the Americas
author:
  - name: Carla Calvo Manosa
    affiliation: Reporting Officer
    email: calvoc@unhcr.org
  - name: Carmen Asenjo
    affiliation: Information Management Officer
    email: asenjo@unhcr.org
output:
...
data <- 
  data %>% 
  filter(is.na(`grRepOp/exReport`) | `grRepOp/exReport`=="" | `grRepOp/exReport`=="no") %>% # remove extraordinary reports from the dataset
  select(id = `_id`,
...
datanow <- datanow [!duplicated(datanow[c(4,5)]),]

```


```{r pipeline, results='asis'}
walk(levels(fct_drop(datanow$region)), ~render_region(datanow, .))
```
