...
subtitle: WEEK `r lubridate::isoweek(lubridate::today())-1`/2021
...
data <- 
  ...
  pivot_longer(-c(id, week, country), names_pattern = "(.+)\\.(.+)", names_to = c("section", "dimension")) %>% 
...
data <- data %>% filter(week == max(week))
