...
# To generate a report for a specific week, change the `target_week` and
# `target_year` values in the `params` section below.
# By default, the report uses the ISO week that is 2 weeks before today.
#
# Example — to produce the Week 20 / 2026 report:
#   target_week: 20
#   target_year: 2026
#
# You can also knit with parameters from RStudio: Knit → Knit with Parameters
# or via rmarkdown::render("report_manual_week.Rmd", params = list(target_week = 20, target_year = 2026))
# ===========================================================================
title: Americas Weekly Highlights
# NOTE: subtitle uses target_week / target_year — change those in params below, not here
subtitle: "WEEK `r params$target_week` / `r params$target_year`"
date: Regional Bureau for the Americas
params:
  # Change these two values to target a specific week/year for the report
  target_week: !r lubridate::isoweek(lubridate::today() - lubridate::weeks(2))
  target_year: !r lubridate::isoyear(lubridate::today() - lubridate::weeks(2))
author:
  - name: Julia Hanby
    affiliation: Government Liaison Officer
    email: hanby@unhcr.org
output:
  pagedown::html_paged:
...
data <- pivoted_data |>
  ...
  mutate(country = case_when(country == "CostaRica" ~ "Costa Rica",
                             ...
                             country == "CaribbeanIsland" ~ "Caribbean Island",
                             country == "guy_nic_sur" ~ "Guyana, Nicaragua and Suriname",
                             country == "CaribbeanCountries" ~ "Caribbean Countries",
  ...
  mutate(region = case_when(country == "Northern Caribbean" ~ "Caribbean",
                            country == "Caribbean Island" ~ "Caribbean",
                            country == "Caribbean Countries" ~ "Caribbean",
                            country == "Curacao" ~ "Caribbean",
                            country == "Guyana, Nicaragua and Suriname" ~ "Central America",
...
```{r datafilter}
# NOTE: For a specific week report, change target_week and target_year in the params section at the top of the file
datanow <- data |> filter(week == params$target_week & year == params$target_year)
